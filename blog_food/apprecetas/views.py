...
def recetas(request, category_id=None):
    ...
    posts = Receta.objects.all().prefetch_related('kategory',
                                                  'kategory__parent')
    ...
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            ...
            return HttpResponseRedirect(
                reverse('apprecetas:receta', args=[post.slug]))
...
def show_subcategory(request, category_id=True, slug=True):
    ...
    post = Receta.objects.filter(
        kategory_id__in=category)  #SE USA CUANDO NO HAY SUBCATEGORIA
    recetas = Receta.objects.filter(kategory__in=category.get_descendants(
        include_self=True))
    categoria = Kategory.objects.filter(parent=None).order_by(
        'slug')  #SALEN SOLO LAS CATEGORIAS
...
def categoriasoriginal(request):
    ...
    children = current_category.get_children()
    cateysubcate = Kategory.objects.filter(
        parent__in=category
    ...
    recetas = Receta.objects.filter(kategory__in=category.get_descendants(
        include_self=True))
    f = Kategory.objects.select_related('parent').prefetch_related(
    ...
    return render(request, 'apprecetas/recetas_saludablesorigial.html',
                  context)
